...
export default function Home() {
  ...
  return (
    <main className="flex flex-col justify-center items-center tracking-wide">
      <div className="w-auto h-[844px] max-w-[390px] bg-ty-background relative md:mt-[50px]">
        <img
          src="/images/ty-logo.PNG"
          alt="today's wildlife logo"
          className="w-[184px] absolute top-[90px] left-1/2 translate-x-[-50%]"
        />
        {!isLoading ? (
          <>
            <div className="mt-[200px] mx-auto pl-[5px] w-[60%] min-w-[250px] font-kosugi-maru">
              <h1 className="text-center text-[22px] mb-[5px]">
                <span className="text-highlight-blue">
                  {!error ? wildlife?.name : 'レッサーパンダ'}
                </span>
              </h1>
              <p className="text-center text-[14px] text-[#1C77A6] mb-[20px] underline underline-offset-4">
                {!error ? wildlife?.habitat : 'ヒマラヤ地域、中国、ネパール、インド'}
              </p>
              <p className="text-[14px] text-[#123866] mb-[15px]">
                {!error
              ...
              <p className="text-[14px] text-[#123866]">
                {!error
                  ? wildlife?.trivia
            ...
            <div className="absolute top-[610px] left-1/2 translate-x-[-50%] flex items-center font-kosugi-maru">
              <div>
                {isBefore ? (
                  <div className="w-[55px] flex justify-center items-center cursor-pointer">
                    <ArrowLeftIcon
                      style={{ color: '#D7494A', fontSize: '40px' }}
                      onClick={getBefore}
                    />
                  </div>
                ) : (
              ...
              <div className="flex flex-col justify-between items-center w-[90px]">
                <a
                  href={`https://ja.wikipedia.org/wiki/${!error ? wildlife?.name : 'レッサーパンダ'}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <img
                    src="/images/ty-icon1.png"
                    alt="today's wildlife icon"
                    className="w-[40px] mx-auto"
                  />
                  <p className="font-kosugi-maru text-[10px] text-[#0e2c52] tracking-tight mt-[-3px] underline underline-offset-2 decoration-2 decoration-[#83BD9C]">
                    check wiki
                  </p>
                </a>
              </div>
          ...
          <>
            ...
            <p className="absolute top-[380px] left-1/2 translate-x-[-50%] font-kosugi-maru tracking-wide text-[#252525]">
              読み込み中
            </p>
        ...
        <p className="absolute top-[685px] left-1/2 translate-x-[-50%] text-[8px]">
